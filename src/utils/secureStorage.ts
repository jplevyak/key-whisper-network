...
export class SecureStorage {
  ...
  private standardKeyWasRetrievedFromStorage = false; // New flag
  ...
  async initializeWithKey(newDerivedKey: CryptoKey, dbManager?: IDBManagerType): Promise<void> {
    ...
    // We should re-encrypt if we have an old key (standard) and we are switching to a derived key.
    // We do NOT rely on standardKeyWasRetrievedFromStorage because the user might have generated a new standard key
    // and added data in the current session before upgrading.
    const shouldReEncrypt = oldKey && !this.isUsingDerivedKey;

    if (shouldReEncrypt) {
      console.log("SecureStorage: Standard key was previously stored and in use. Re-encrypting application data with new derived key.");
    ...
    this.standardKeyWasRetrievedFromStorage = false; // Reset flag as derived key is now active
  ...
  async init(): Promise<void> {
    ...
    if (storedKey) {
      ...
      this.standardKeyWasRetrievedFromStorage = true; // Mark that key was retrieved
      this.isUsingDerivedKey = false; // Ensure this is false for a standard key
      console.log("SecureStorage.init: Initialized with standard key retrieved from storage.");
      return;
    }

    // Generate new key if none exists
    console.log("SecureStorage.init: No stored standard key found, generating a new one.");
    ...
    this.standardKeyWasRetrievedFromStorage = false; // Mark that key was newly generated
    this.isUsingDerivedKey = false; // Ensure this is false for a standard key
    console.log("SecureStorage.init: New standard key generated and stored.");
  }
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        request.onsuccess = () => {
          ...
          this.standardKeyWasRetrievedFromStorage = false; // Reset flag
          resolve();
        };
        request.onblocked = () => {
